...
all application reads use `legacy_access_level`. Run the idempotent
reconciliation check after migrations and immediately before activating a
release:
...
fields are missing or unsupported legacy access values exist. It synchronizes
the additive fields while `users.role` exists and becomes a successful no-op
after Phase B removes that column.
...
5. The activation script extracts to `/app/data/releases/<sha>`, links shared `.env` and Laravel storage, validates OIDC/LDAP readiness, runs migrations and cache warm-up, reconciles access compatibility data, then atomically updates `/app/data/current`.
...
- `users`: identity source, transitional legacy access level, rollback-compatible role, and LDAP groups
