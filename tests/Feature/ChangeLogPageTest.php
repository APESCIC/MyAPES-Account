...
class ChangeLogPageTest extends TestCase
{
    ...
    public function test_guest_can_read_the_complete_progressive_change_log(): void
    {
        ...
        $response
            ...
            ->assertSeeText('Current version v0.9.1')
            ->assertSee('data-change-log', false)
            ->assertSee('data-change-log-controls hidden', false)
            ->assertSee('href="#release-v0-9-1"', false)
            ->assertSee('<details', false)
            ->assertSeeText('Compatible public-account lifecycle foundation');

        $this->assertSame(18, substr_count($response->getContent(), 'data-release-record'));

        foreach (['0.9.1', '0.9.0', '0.8.3', '0.8.2', '0.8.1', '0.8.0', '0.7.1', '0.7.0', '0.6.1', '0.6.0', '0.5.0', '0.4.2', '0.4.1', '0.4.0', '0.3.0', '0.2.1', '0.2.0', '0.1.0'] as $version) {
    ...
    public function test_shared_layout_exposes_the_accessible_current_version_link(): void
    {
        foreach (['/', '/register', '/staff/login', '/change-log'] as $path) {
            $this->get($path)
                ...
                ->assertSee('aria-label="View the MyAPES Account change log for version v0.9.1"', false)
                ->assertSeeText('v0.9.1');
        }
