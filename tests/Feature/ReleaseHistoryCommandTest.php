...
class ReleaseHistoryCommandTest extends TestCase
{
    public function test_source_controlled_history_contains_the_required_release_sequence(): void
    {
        ...
        $this->assertSame('0.13.1', $repository->version());
        $this->assertSame('0.13.1', $repository->current()['version']);
        $this->assertSame(
            ['0.13.1', '0.13.0', '0.12.1', '0.12.0', '0.11.0', '0.10.0', '0.9.2', '0.9.1', '0.9.0', '0.8.3', '0.8.2', '0.8.1', '0.8.0', '0.7.1', '0.7.0', '0.6.1', '0.6.0', '0.5.0', '0.4.2', '0.4.1', '0.4.0', '0.3.0', '0.2.1', '0.2.0', '0.1.0'],
            array_column($releases, 'version'),
        );
        $this->assertSame('2026-08-11', $releases[0]['date']);
        ...
        $this->assertSame('2026-08-08', $releases[7]['date']);
        $this->assertSame('2026-08-07', $releases[8]['date']);
        $this->assertSame('2026-08-06', $releases[9]['date']);
        ...
        $this->assertSame('2026-07-28', $releases[13]['date']);
        ...
        foreach (array_slice($releases, 17) as $release) {
        ...
        $this->assertSame(
            [
                [
                    'label' => 'Issue #42',
                    'url' => 'https://github.com/APESCIC/MyAPES-Account/issues/42',
                ],
                [
                    'label' => 'PR #43',
                    'url' => 'https://github.com/APESCIC/MyAPES-Account/pull/43',
                ],
            ],
            $current['references'],
        );
        ...
        $previous = $releases[1];
        $this->assertSame('minor', $previous['type']);
        $this->assertSame(
            [[
                'label' => 'Issue #12',
        ...
        $this->assertStringContainsString(
            'database recovery',
            strtolower(implode(' ', $previous['known_limitations'])),
        );
    ...
    public function test_validation_command_accepts_the_bootstrap_history(): void
    {
        $this->artisan('myapes:changelog-validate')
            ->expectsOutputToContain('Release history is valid at v0.13.1')
            ->assertSuccessful();
    }
