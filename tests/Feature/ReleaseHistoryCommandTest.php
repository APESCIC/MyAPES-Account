...
class ReleaseHistoryCommandTest extends TestCase
{
    public function test_source_controlled_history_contains_the_required_release_sequence(): void
    {
        ...
        $this->assertSame('0.9.0', $repository->version());
        $this->assertSame('0.9.0', $repository->current()['version']);
        $this->assertSame(
        ...
        $this->assertSame('2026-07-28', $releases[5]['date']);
        ...
        $this->assertSame(
            [[
                'label' => 'Issue #11',
                'url' => 'https://github.com/APESCIC/MyAPES-Account/issues/11',
            ]],
            $current['references'],
        );
    ...
    public function test_validation_command_accepts_the_bootstrap_history(): void
    {
        $this->artisan('myapes:changelog-validate')
            ->expectsOutputToContain('Release history is valid at v0.9.0')
            ->assertSuccessful();
    }
