...
class ReleaseHistoryCommandTest extends TestCase
{
    public function test_source_controlled_history_contains_the_required_release_sequence(): void
    {
        ...
        $this->assertSame('0.8.3', $repository->version());
        $this->assertSame('0.8.3', $repository->current()['version']);
        $this->assertSame(
            ['0.8.3', '0.8.2', '0.8.1', '0.8.0', '0.7.1', '0.7.0', '0.6.1', '0.6.0', '0.5.0', '0.4.2', '0.4.1', '0.4.0', '0.3.0', '0.2.1', '0.2.0', '0.1.0'],
        ...
        $this->assertSame('2026-07-28', $releases[4]['date']);
        ...
        foreach (array_slice($releases, 9) as $release) {
            $this->assertSame('2026-07-24', $release['date']);
        }

        foreach (array_slice($releases, 8) as $release) {
        ...
        $this->assertSame(
            [[
                'label' => 'Issue #10',
                'url' => 'https://github.com/APESCIC/MyAPES-Account/issues/10',
            ]],
            $current['references'],
        );
    ...
    public function test_validation_command_accepts_the_bootstrap_history(): void
    {
        $this->artisan('myapes:changelog-validate')
            ->expectsOutputToContain('Release history is valid at v0.8.3')
            ->assertSuccessful();
    }
