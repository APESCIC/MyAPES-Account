...
class ReleaseHistoryCommandTest extends TestCase
{
    public function test_source_controlled_history_contains_the_required_release_sequence(): void
    {
        ...
        $this->assertSame('0.9.1', $repository->version());
        $this->assertSame('0.9.1', $repository->current()['version']);
        $this->assertSame(
            ['0.9.1', '0.9.0', '0.8.3', '0.8.2', '0.8.1', '0.8.0', '0.7.1', '0.7.0', '0.6.1', '0.6.0', '0.5.0', '0.4.2', '0.4.1', '0.4.0', '0.3.0', '0.2.1', '0.2.0', '0.1.0'],
            array_column($releases, 'version'),
        );
        $this->assertSame('2026-08-08', $releases[0]['date']);
        $this->assertSame('2026-08-07', $releases[1]['date']);
        $this->assertSame('2026-08-06', $releases[2]['date']);
        $this->assertSame('2026-08-05', $releases[3]['date']);
        $this->assertSame('2026-08-05', $releases[4]['date']);
        $this->assertSame('2026-08-05', $releases[5]['date']);
        $this->assertSame('2026-07-28', $releases[6]['date']);
        $this->assertSame('2026-07-27', $releases[7]['date']);
        $this->assertSame('2026-07-27', $releases[8]['date']);
        $this->assertSame('2026-07-27', $releases[9]['date']);
        $this->assertSame('2026-07-27', $releases[10]['date']);

        foreach (array_slice($releases, 11) as $release) {
            $this->assertSame('2026-07-24', $release['date']);
        }

        foreach (array_slice($releases, 10) as $release) {
    ...
    public function test_validation_command_accepts_the_bootstrap_history(): void
    {
        $this->artisan('myapes:changelog-validate')
            ->expectsOutputToContain('Release history is valid at v0.9.1')
            ->assertSuccessful();
    }
